...
def generate_og_image(colors):
    ...
    # Fixed grid and Perlin noise parameters
    cell_size = 30
    cell_gap = 10
    radius = 4
...
def main():
    ...
    try:
        css_template = css_env.get_template(CSS_TEMPLATE_NAME) # expects 'styles.css' in 'styles/' dir
        # Pass theme_colors_dict as 'theme_colors' to the CSS template
        output_css = css_template.render(theme_colors=theme_colors) 
        os.makedirs(os.path.join(OUTPUT_DIR, 'styles'), exist_ok=True) # Ensure _site/styles exists
